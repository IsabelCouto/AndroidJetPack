...
class NotificationsHelper(val context: Context) {
    ...
    fun createNotification(){
        ...
        val icon = BitmapFactory.decodeResource(context.resources, R.drawable.ic_dog)
        val notification = NotificationCompat.Builder(context, CHANNEL_ID).setSmallIcon(R.drawable.dog_icon)
            .setLargeIcon(icon).setContentTitle("Dogs retrieved")
            .setContentText("This notification has some content").setStyle(NotificationCompat.BigPictureStyle().bigPicture(icon).bigLargeIcon(null))
